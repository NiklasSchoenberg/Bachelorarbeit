...
output_folder = '3d Plots deepL'
os.makedirs(output_folder, exist_ok=True)

for tool_name in tool_names:

    df_tool = pd.read_csv('deepLKommentareToolScores.csv')
    ...
    plt.savefig(os.path.join(output_folder, f'deepL3D_Surface_Plot_{tool_name}.png'))
    plt.close()

f1_scores_df = pd.DataFrame(f1_scores_list)

f1_scores_df.to_csv('deepLf1_scores_all_tools.csv', index=False)
